...
//  Dorsal (y > cy) and ventral (y < cy) sheets are driven a quarter period apart,
...
class DistributedAttachmentForce : public BaseForcePrior<SPHBody>
{
  public:
    ...
    void computeLinkKinematicsAndForce(const DistributedAttachmentLink &link, Vecd &displacement, Vecd &link_force)
    {
        ...
        link_force = particle_stiffness * displacement + particle_damping * velocity_difference;
    }
  ...
  public:
    ...
    SimTK::SpatialVec reduce(size_t index_i, Real dt = 0.0)
    {
        ...
        for (const DistributedAttachmentLink &link : attachment_network_.linksForParticle(index_i))
        {
            ...
            Vec3d target_buffer_pos, target_buffer_vel, target_acc, target_normal;
            simbody_state_.findStationLocationVelocityAndAccelerationInGround(
                upgradeToVec3d(link.buffer_station_initial_), Vec3d::UnitY(),
                target_buffer_pos, target_buffer_vel, target_acc, target_normal);

            Vecd displacement = degradeToVecd(target_buffer_pos) - pos_[index_i];
            Vecd velocity_difference = degradeToVecd(target_buffer_vel) - vel_[index_i];
            Real particle_stiffness = mass_[index_i] * omega_attach * omega_attach;
            Real particle_damping = 2.0 * zeta_attach * mass_[index_i] * omega_attach;
            Vecd link_force = particle_stiffness * displacement + particle_damping * velocity_difference;

            Vec3d target_bone_pos, target_bone_vel;
            simbody_state_.findStationLocationVelocityAndAccelerationInGround(
                upgradeToVec3d(link.bone_station_initial_), Vec3d::UnitY(),
                target_bone_pos, target_bone_vel, target_acc, target_normal);

            SimTKVec3 reaction_force = -EigenToSimTK(upgradeToVec3d(link_force));
            SimTKVec3 lever_arm = EigenToSimTK(target_bone_pos) - current_mobod_origin_location_;
            total_reaction_force += reaction_force;
            total_reaction_torque += SimTK::cross(lever_arm, reaction_force);
        }
